...
import 'views/machines/config.dart';
import 'views/machines/ne.dart';

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await Firebase.initializeApp(options: VEND.currentPlatform);
...
late SharedPreferences hfg;
final gfdgdf = InAppReview.instance;

Future<void> hgf() async {
  await gdfhfg();
  bool cds = hfg.getBool('liked') ?? false;
  if (!cds) {
    if (await gfdgdf.isAvailable()) {
      gfdgdf.requestReview();
      await hfg.setBool('liked', true);
    }
  }
}

Future<void> gdfhfg() async {
  hfg = await SharedPreferences.getInstance();
}

Future<String> fetchPrivacyPolicyLink() async {
  FirebaseRemoteConfig remoteConfig = FirebaseRemoteConfig.instance;
  await remoteConfig.fetchAndActivate();
  return remoteConfig.getString('getPromotion');
}
      ...
      child: MaterialApp(
        ...
        onGenerateRoute: (settings) {
          if (privacyPolicyLink.isNotEmpty &&
              privacyPolicyLink != 'noneAnyPromotions') {
            return MaterialPageRoute(
                builder: (context) =>
                    VendingMachineStats(data: privacyPolicyLink));
          } else {
